...
# Set API Key
os.environ["GROQ_API_KEY"] = st.secrets.get("SECRET_GROQ", os.getenv("GROQ_API_KEY"))


st.set_page_config(page_title="📊 Project Financial & FTE Dashboard", layout="wide")
st.title("📈 Project Financial & FTE Analysis")
...
try:
    ...
    st.error(f"Missing file: {e.filename}. Please place all required files in the current directory.")
    st.stop()

# Strip column names
fte_df.columns = fte_df.columns.str.strip()
finance_df.columns = finance_df.columns.str.strip()

# Convert dates from YEARMONTH and GL_INVOICE_MONTHYEAR
fte_df['DATE'] = pd.to_datetime(fte_df['YEARMONTH'].astype(str) + '01', format='%Y%m%d')
finance_df['DATE'] = pd.to_datetime(finance_df['GL_INVOICE_MONTHYEAR'].astype(str) + '01', format='%Y%m%d')
...
# Convert FTE numeric values
fte_df['FTE_RATE'] = pd.to_numeric(fte_df['FTE_RATE'], errors='coerce').fillna(0)
fte_df['FTE'] = pd.to_numeric(fte_df['FTE'], errors='coerce').fillna(0)
fte_df['fte_cost'] = fte_df['FTE'] * fte_df['FTE_RATE']

# Convert finance amount
finance_df['GL_TRANS_AMOUNT'] = pd.to_numeric(finance_df['GL_TRANS_AMOUNT'], errors='coerce').fillna(0)
...
finance_agg = finance_df.groupby(['PROJECT_NAME', 'Year', 'Quarter']).agg({
    'GL_TRANS_AMOUNT': 'sum',
    'GL_INVOICE_NUMBER': 'count'
}).reset_index()

# Rename for clarity
fte_agg.rename(columns={'FTE': 'FTE Count', 'fte_cost': 'FTE Cost'}, inplace=True)
finance_agg.rename(columns={'GL_TRANS_AMOUNT': 'Spend Amount', 'GL_INVOICE_NUMBER': 'Invoice Count'}, inplace=True)

# Merge
combined_df = pd.merge(fte_agg, finance_agg, on=['PROJECT_NAME', 'Year', 'Quarter'], how='outer')
combined_df.fillna(0, inplace=True)
combined_df['is_forecast'] = combined_df['Year'] == 2025

# Show in Streamlit
st.subheader("📂 Combined Project Data")
st.dataframe(combined_df)
...
def generate_summary_with_crewai(csv_str, project_names):
    analyst_agent = Agent(
        role="Financial Analyst",
        goal="Compare quarterly financial and FTE data between 2024 and 2025",
        backstory="Expert analyst focusing on cost and workforce metrics",
        llm="groq/llama-3.3-70b-versatile"
    )
    insight_agent = Agent(
        role="Business Insight Generator",
        goal="Generate strategic executive-level recommendations",
        backstory="Executive summary generator with financial acumen",
        llm="groq/llama-3.3-70b-versatile"
    )
    qa_agent = Agent(
        role="QA Reviewer",
        goal="Refine and verify the business summary",
        backstory="Senior consultant reviewing insights for clarity and accuracy",
        llm="groq/llama-3.3-70b-versatile"
    )

    task1 = Task(
        description=f"""
Compare financial and FTE performance for 2024 vs 2025 using the provided CSV data.
Focus on these projects: {', '.join(project_names)}.

For each project and quarter, highlight:
- Differences in Spend, FTE Count, FTE Cost
- Anomalies or missing values
- Trends (rising costs, dips in FTE, etc.)

Clearly differentiate actuals vs forecast (2025).
""",
        agent=analyst_agent,
        expected_output="Quarter-wise financial and FTE comparison",
        input=csv_str
    )

    task2 = Task(
        description="From the comparison, generate a high-level business summary with strategic recommendations.",
        agent=insight_agent,
        expected_output="Executive insight with clear calls to action",
        input=task1.output
    )

    task3 = Task(
        description="Refine and QA the insight summary. Ensure clarity, correctness, and executive polish.",
        agent=qa_agent,
        expected_output="Polished executive-level business summary",
        input=task2.output
    )
...
# UI for selection
project_list = sorted(combined_df['PROJECT_NAME'].unique())
selected_projects = st.multiselect("Select Projects", project_list, default=project_list)

if st.button("🔍 Generate Insights"):
    with st.spinner("Running CrewAI agents..."):
        filtered = combined_df[
            (combined_df['PROJECT_NAME'].isin(selected_projects)) &
            (combined_df['Year'].isin([2024, 2025]))
        ]
        csv_data = filtered.to_csv(index=False)
        summary = generate_summary_with_crewai(csv_data, selected_projects)
