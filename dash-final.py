...
def generate_report(df):
    ...
    task1 = Task(
        description=f"""
Compare 2024 vs 2025 data per quarter (Q1–Q4) for project: {selected_project}.
Use metrics: total_fte, avg_fte_rate, fte_cost, total_spend, invoice_count.
Indicate if 2025 values are forecasted using `is_forecast` column.
Only use project name: {selected_project}
        """,
        agent=analyst_agent,
        expected_output="Quarterly comparison report.",
        input=csv_str
    )

    task2 = Task(
        description="Generate strategic executive insights based on the comparison.",
        agent=insight_agent,
        expected_output="Executive-level insight summary.",
        input=task1.output
    )

    task3 = Task(
        description="Clean, verify, and format the summary for final delivery.",
        agent=qa_agent,
        expected_output="Final QA-approved report.",
        input=task2.output
    )
